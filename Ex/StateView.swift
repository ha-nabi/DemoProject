...
/**
 ...
 - 상태 객체와 다르게 관찰되는 객체 참조는 그것을 선언한 뷰가 소유하지 않으므로
   사용되는 동안에 (예, 뷰의 재렌더링) SwiftUI 시스템에 의해 파괴되거나 다시 생성될 위험이 있다.
 - @StateObject 를 사용하면 참조가 선언한 뷰가 참조를 소유하므로, 해당 참조가 선언된 뷰에 의해
   계속해서 필요로 하는 동안에 유지된다.
 */
...
/**
 ...
 SwiftUI 환경에 저장되어 뷰에서 뷰로 전달할 필요없이 모든 자식 뷰가 접근할 수 있다.
  ...
  - 작업을 수행하는 뷰의 상의 뷰에서 Observable 객체의 인스턴스 초기화
